#!/bin/bash
###############################################################################
# BitcoinPurple Core - Linux x86_64 Docker Build Wrapper
###############################################################################
...
OUTPUT_DIR="$PROJECT_ROOT/build-output/linux-x86_64"
...
docker build $NO_CACHE -t bitcoinpurple-builder:linux-x86_64 -f "$SCRIPT_DIR/Dockerfile" "$SCRIPT_DIR"

# Create ccache volume if needed
if ! docker volume inspect bitcoinpurple_ccache_linux >/dev/null 2>&1; then
    log_info "Creating ccache volume..."
    docker volume create bitcoinpurple_ccache_linux
fi
...
docker run --rm \
    ...
    -v "$OUTPUT_DIR:/output" \
    -v bitcoinpurple_ccache_linux:/ccache \
    -e GIT_COMMIT="$ACTUAL_COMMIT" \
    bitcoinpurple-builder:linux-x86_64
