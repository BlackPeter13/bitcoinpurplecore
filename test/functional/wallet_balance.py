...
class WalletTest(BitcoinPurpleTestFramework):
    ...
    def run_test(self):
        ...
        if not self.options.descriptors:
            ...
            assert_equal(self.nodes[0].getbalance(minconf=0, include_watchonly=True), 1)
            assert_equal(self.nodes[0].getbalance("*", 1, True), 1)
        assert_equal(self.nodes[1].getbalance(minconf=0, include_watchonly=True), 1)

        # Send 0.4 BTCP from 0 to 1 and 0.6 BTCP from 1 to 0.
        txs = create_transactions(self.nodes[0], self.nodes[1].getnewaddress(), Decimal('0.79'), [Decimal('0.01')])
        self.nodes[0].sendrawtransaction(txs[0]['hex'])
        self.nodes[1].sendrawtransaction(txs[0]['hex'])  # sending on both nodes is faster than waiting for propagation

        self.sync_all()
        txs = create_transactions(self.nodes[1], self.nodes[0].getnewaddress(), Decimal('0.99'), [Decimal('0.01'), Decimal('0.02')])
        ...
        def test_balances(*, fee_node_1=0):
            ...
            expected_balances_0 = {'mine':      {'immature':          Decimal('0E-8'),
                                                 'trusted':           Decimal('0.20'),  # change from node 0's send (1 - 0.79 - 0.01)
                                                 'untrusted_pending': Decimal('0.99')},
                                   'watchonly': {'immature':          Decimal('100'),
                                                 'trusted':           Decimal('1.0'),
                                                 'untrusted_pending': Decimal('0E-8')}}
            expected_balances_1 = {'mine':      {'immature':          Decimal('0E-8'),
                                                 'trusted':           Decimal('0E-8'),  # node 1's send had an unsafe input
                                                 'untrusted_pending': Decimal('0.80') - fee_node_1}}  # Doesn't include output of node 0's send since it was spent
            if self.options.descriptors:
                del expected_balances_0["watchonly"]
            assert_equal(self.nodes[0].getbalances(), expected_balances_0)
            assert_equal(self.nodes[1].getbalances(), expected_balances_1)
            # getbalance without any arguments includes unconfirmed transactions, but not untrusted transactions
            assert_equal(self.nodes[0].getbalance(), Decimal('0.20'))  # change from node 0's send
            assert_equal(self.nodes[1].getbalance(), Decimal('0'))  # node 1's send had an unsafe input
            # Same with minconf=0
            assert_equal(self.nodes[0].getbalance(minconf=0), Decimal('0.20'))
            ...
            assert_equal(self.nodes[0].getunconfirmedbalance(), Decimal('0.99'))  # output of node 1's spend
            assert_equal(self.nodes[1].getunconfirmedbalance(), Decimal('0.80') - fee_node_1)  # Doesn't include output of node 0's send since it was spent
            # getwalletinfo.unconfirmed_balance
            assert_equal(self.nodes[0].getwalletinfo()["unconfirmed_balance"], Decimal('0.99'))
            assert_equal(self.nodes[1].getwalletinfo()["unconfirmed_balance"], Decimal('0.80') - fee_node_1)
        ...
        balance_node0 = Decimal('1.19')  # node 1's send (0.6) plus change from node 0's send (0.59)
        balance_node1 = Decimal('0.78')  # change from node 1's send (1 + 0.4 - 0.6 - 0.02)
        ...
        for _ in range(3):
            ...
            txid = self.nodes[0].sendtoaddress(self.nodes[0].getnewaddress(), 1.8)
        ...
        tx_replace = tx_orig.replace(
            struct.pack("<q", 180_000_000).hex(),
            struct.pack("<q", 80_000_000).hex(),
        )
