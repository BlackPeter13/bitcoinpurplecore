...
class RPCGenerateTest(BitcoinPurpleTestFramework):
    ...
    def test_generatetoaddress(self):
        self.generatetoaddress(self.nodes[0], 1, 'Pf4NKo7hpzFvn8mx8Mdg39C1M6zxRnhb3J')
    ...
    def test_generateblock(self):
        ...
        combo_address = 'rbtcp1qw508d6qejxtdg4y5r3zarvary0c5xw7kvj3pyl'
        ...
        combo_address = 'Pd1yHVDokH8YpgZmJ2JERCk2z2ZU3V2U38'
        hash = self.generateblock(node, 'combo(' + combo_key + ')', [])['hash']
        block = node.getblock(hash, 2)
        assert_equal(len(block['tx']), 1)
        assert_equal(block['tx'][0]['vout'][0]['scriptPubKey']['address'], combo_address)
