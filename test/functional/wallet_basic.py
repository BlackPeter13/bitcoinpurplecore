...
class WalletTest(BitcoinPurpleTestFramework):
    ...
    def run_test(self):
        ...
        # Send 0.21 BTCP from 0 to 2 using sendtoaddress call.
        self.nodes[0].sendtoaddress(self.nodes[2].getnewaddress(), Decimal("0.11"))
        mempool_txid = self.nodes[0].sendtoaddress(self.nodes[2].getnewaddress(), Decimal("0.10"))
        ...
        # but 0.10 will go to node2 and the rest will go to node0
        balance = self.nodes[0].getbalance()
        assert_equal(set([txout1['value'], txout2['value']]), set([Decimal("0.10"), balance]))
        walletinfo = self.nodes[0].getwalletinfo()
        ...
        tx = self.nodes[1].createrawtransaction([unspent_0], { self.nodes[1].getnewaddress() : Decimal("0.1") })
        ...
        # node0 should end up with 2 btcp in block rewards plus fees, but
        # minus the 0.21 plus fees sent to node2
        assert_equal(self.nodes[0].getbalance(), Decimal("2") - Decimal("0.21"))
        ...
        for utxo in node0utxos:
            ...
            outputs[self.nodes[2].getnewaddress()] = utxo["amount"] - Decimal("0.03")
        ...
        assert_equal(self.nodes[2].getbalance(), Decimal("1.94"))

        # Verify that a spent output cannot be locked anymore
        spent_0 = {"txid": node0utxos[0]["txid"], "vout": node0utxos[0]["vout"]}
        assert_raises_rpc_error(-8, "Invalid parameter, expected unspent output", self.nodes[0].lockunspent, False, [spent_0])

        # Send 0.10 BTCP normal
        address = self.nodes[0].getnewaddress("test")
        fee_per_byte = Decimal('0.001') / 1000
        self.nodes[2].settxfee(fee_per_byte * 1000)
        txid = self.nodes[2].sendtoaddress(address, Decimal("0.10"), "", "", False)
        self.generate(self.nodes[2], 1, sync_fun=lambda: self.sync_all(self.nodes[0:3]))
        node_2_bal = self.check_fee_amount(self.nodes[2].getbalance(), Decimal("1.84"), fee_per_byte, self.get_vsize(self.nodes[2].gettransaction(txid)['hex']))
        assert_equal(self.nodes[0].getbalance(), Decimal("0.10"))

        # Send 0.10 BTCP with subtract fee from amount
        txid = self.nodes[2].sendtoaddress(address, Decimal("0.10"), "", "", True)
        self.generate(self.nodes[2], 1, sync_fun=lambda: self.sync_all(self.nodes[0:3]))
        node_2_bal -= Decimal("0.10")
        assert_equal(self.nodes[2].getbalance(), node_2_bal)
        node_0_bal = self.check_fee_amount(self.nodes[0].getbalance(), Decimal("0.20"), fee_per_byte, self.get_vsize(self.nodes[2].gettransaction(txid)['hex']))

        self.log.info("Test sendmany")

        # Sendmany 0.10 BTCP
        txid = self.nodes[2].sendmany('', {address: Decimal("0.10")}, 0, "", [])
        self.generate(self.nodes[2], 1, sync_fun=lambda: self.sync_all(self.nodes[0:3]))
        node_0_bal += Decimal("0.10")
        node_2_bal = self.check_fee_amount(self.nodes[2].getbalance(), node_2_bal - Decimal("0.10"), fee_per_byte, self.get_vsize(self.nodes[2].gettransaction(txid)['hex']))
        assert_equal(self.nodes[0].getbalance(), node_0_bal)

        # Sendmany 0.10 BTCP with subtract fee from amount
        txid = self.nodes[2].sendmany('', {address: Decimal("0.10")}, 0, "", [address])
        self.generate(self.nodes[2], 1, sync_fun=lambda: self.sync_all(self.nodes[0:3]))
        node_2_bal -= Decimal("0.10")
        assert_equal(self.nodes[2].getbalance(), node_2_bal)
        node_0_bal = self.check_fee_amount(self.nodes[0].getbalance(), node_0_bal + Decimal("0.10"), fee_per_byte, self.get_vsize(self.nodes[2].gettransaction(txid)['hex']))
        ...
        txid = self.nodes[2].sendmany(amounts={address: Decimal("0.10")}, fee_rate=str(fee_rate_sat_vb))
        self.generate(self.nodes[2], 1, sync_fun=lambda: self.sync_all(self.nodes[0:3]))
        balance = self.nodes[2].getbalance()
        node_2_bal = self.check_fee_amount(balance, node_2_bal - Decimal("0.10"), explicit_fee_rate_btcp_kvb, self.get_vsize(self.nodes[2].gettransaction(txid)['hex']))
        ...
        usp = self.nodes[1].listunspent(query_options={'minimumAmount': '0.9'})[0]
        inputs = [{"txid": usp['txid'], "vout": usp['vout']}]
        outputs = {self.nodes[1].getnewaddress(): Decimal("0.8"), self.nodes[0].getnewaddress(): Decimal("0.1111")}

        raw_tx = self.nodes[1].createrawtransaction(inputs, outputs).replace("7086a900", "00000000")  # replace 0.1111 with 0.0 (int32)
        ...
        if not self.options.descriptors:
            ...
            assert_raises_rpc_error(-5, "Bech32m addresses cannot be imported into legacy wallets", self.nodes[0].importaddress, "rbtcp1p0xlxvlhemja6c4dqv22uapctqupfhlxm9h8z3k2e72q4k9hcz7vqmeq5aq")

            # Import address and private key to check correct behavior of spendable unspents
            # 1. Send some coins to generate new UTXO
            address_to_import = self.nodes[2].getnewaddress()
            txid = self.nodes[0].sendtoaddress(address_to_import, Decimal("0.01"))
            ...
            assert prebalance > Decimal("0.002")
            ...
            for key in ["totalFee", "feeRate"]:
                assert_raises_rpc_error(-8, "Unknown named parameter key", self.nodes[2].sendtoaddress, address=address, amount=Decimal("0.01"), fee_rate=1, key=1)

            # Test setting explicit fee rate just below the minimum.
            self.log.info("Test sendtoaddress raises 'fee rate too low' if fee_rate of 0.99999999 is passed")
            assert_raises_rpc_error(-6, "Fee rate (0.999 sat/vB) is lower than the minimum fee rate setting (1.000 sat/vB)",
                self.nodes[2].sendtoaddress, address=address, amount=Decimal("0.01"), fee_rate=0.999)

            self.log.info("Test sendtoaddress raises if an invalid fee_rate is passed")
            # Test fee_rate with zero values.
            msg = "Fee rate (0.000 sat/vB) is lower than the minimum fee rate setting (1.000 sat/vB)"
            for zero_value in [0, 0.000, 0.00000000, "0", "0.000", "0.00000000"]:
                assert_raises_rpc_error(-6, msg, self.nodes[2].sendtoaddress, address=address, amount=Decimal("0.01"), fee_rate=zero_value)
            msg = "Invalid amount"
            # Test fee_rate values that don't pass fixed-point parsing checks.
            for invalid_value in ["", 0.000000001, 1e-09, 1.111111111, 1111111111111111, "31.999999999999999999999"]:
                assert_raises_rpc_error(-3, msg, self.nodes[2].sendtoaddress, address=address, amount=Decimal("0.01"), fee_rate=invalid_value)
            # Test fee_rate values that cannot be represented in sat/vB.
            for invalid_value in [0.0001, 0.00000001, 0.00099999, 31.99999999, "0.0001", "0.00000001", "0.00099999", "31.99999999"]:
                assert_raises_rpc_error(-3, msg, self.nodes[2].sendtoaddress, address=address, amount=Decimal("0.01"), fee_rate=invalid_value)
            # Test fee_rate out of range (negative number).
            assert_raises_rpc_error(-3, OUT_OF_RANGE, self.nodes[2].sendtoaddress, address=address, amount=Decimal("0.01"), fee_rate=-1)
            # Test type error.
            for invalid_value in [True, {"foo": "bar"}]:
                assert_raises_rpc_error(-3, NOT_A_NUMBER_OR_STRING, self.nodes[2].sendtoaddress, address=address, amount=Decimal("0.01"), fee_rate=invalid_value)

            self.log.info("Test sendtoaddress raises if an invalid conf_target or estimate_mode is passed")
            for target, mode in product([-1, 0, 1009], ["economical", "conservative"]):
                assert_raises_rpc_error(-8, "Invalid conf_target, must be between 1 and 1008",  # max value of 1008 per src/policy/fees.h
                    self.nodes[2].sendtoaddress, address=address, amount=Decimal("0.01"), conf_target=target, estimate_mode=mode)
            for target, mode in product([-1, 0], ["btcp/kb", "sat/b"]):
                assert_raises_rpc_error(-8, 'Invalid estimate_mode parameter, must be one of: "unset", "economical", "conservative"',
                    self.nodes[2].sendtoaddress, address=address, amount=Decimal("0.01"), conf_target=target, estimate_mode=mode)
        ...
        address_info = self.nodes[0].getaddressinfo("Pf4NKo7hpzFvn8mx8Mdg39C1M6zxRnhb3J")
        assert_equal(address_info['address'], "Pf4NKo7hpzFvn8mx8Mdg39C1M6zxRnhb3J")
        ...
        txid = self.nodes[0].sendtoaddress(destination, Decimal("0.000123"))
        ...
        address = self.nodes[0].getnewaddress("test")
        txid_feeReason_one = self.nodes[2].sendtoaddress(address=address, amount=Decimal("0.05"), verbose=True)
        assert_equal(txid_feeReason_one["fee_reason"], "Fallback fee")
        txid_feeReason_two = self.nodes[2].sendmany(dummy='', amounts={address: Decimal("0.05")}, verbose=True)
        assert_equal(txid_feeReason_two["fee_reason"], "Fallback fee")
        self.log.info("Test send* RPCs with verbose=False")
        txid_feeReason_three = self.nodes[2].sendtoaddress(address=address, amount=Decimal("0.05"), verbose=False)
        assert_equal(self.nodes[2].gettransaction(txid_feeReason_three)['txid'], txid_feeReason_three)
        txid_feeReason_four = self.nodes[2].sendmany(dummy='', amounts={address: Decimal("0.05")}, verbose=False)
        ...
        default_wallet.sendtoaddress(zeroconf_wallet.getnewaddress(), Decimal("0.01"))
        self.generate(self.nodes[0], 1, sync_fun=self.no_op)
        ...
        assert_raises_rpc_error(-6, "Insufficient funds", zeroconf_wallet.sendtoaddress, zeroconf_wallet.getnewaddress(), Decimal("0.005"))
        ...
        zeroconf_wallet.sendtoaddress(zeroconf_wallet.getnewaddress(), Decimal("0.005"))
