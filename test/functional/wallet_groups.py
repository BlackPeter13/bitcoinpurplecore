...
class WalletGroupTest(BitcoinPurpleTestFramework):
    ...
    def run_test(self):
        ...
        [self.nodes[0].sendtoaddress(addr, 0.1) for addr in addrs]
        [self.nodes[0].sendtoaddress(addr, 0.05) for addr in addrs]
        ...
        txid1 = self.nodes[1].sendtoaddress(self.nodes[0].getnewaddress(), 0.02)
        ...
        txid2 = self.nodes[2].sendtoaddress(self.nodes[0].getnewaddress(), 0.02)
        ...
        # Sending 0.14 btcp should pick one 0.1 + one more. For node #1,
        # this could be (A / B0 / C0) + (B1 / C1 / D). We ensure that it is
        # B0 + B1 or C0 + C1, because this avoids partial spends while not being
        # detrimental to transaction cost
        txid3 = self.nodes[1].sendtoaddress(self.nodes[0].getnewaddress(), 0.14)
        ...
        tx4_ungrouped_fee = 3080 if self.options.descriptors else 2820
        tx4_grouped_fee = 4240 if self.options.descriptors else 4160
        tx5_6_ungrouped_fee = 5380 if self.options.descriptors else 5520
        tx5_6_grouped_fee = 7680 if self.options.descriptors else 8240
        node3_grouping_decision = "grouped" if self.options.descriptors else "non-grouped"

        self.log.info("Test wallet option maxapsfee")
        addr_aps = self.nodes[3].getnewaddress()
        self.nodes[0].sendtoaddress(addr_aps, 0.1)
        self.nodes[0].sendtoaddress(addr_aps, 0.1)
        self.generate(self.nodes[0], 1)
        with self.nodes[3].assert_debug_log([f'Fee non-grouped = {tx4_ungrouped_fee}, grouped = {tx4_grouped_fee}, using grouped']):
            txid4 = self.nodes[3].sendtoaddress(self.nodes[0].getnewaddress(), 0.01)
        ...
        [self.nodes[0].sendtoaddress(addr_aps2, 0.1) for _ in range(5)]
        self.generate(self.nodes[0], 1)
        with self.nodes[3].assert_debug_log([f'Fee non-grouped = {tx5_6_ungrouped_fee}, grouped = {tx5_6_grouped_fee}, using {node3_grouping_decision}']):
            txid5 = self.nodes[3].sendtoaddress(self.nodes[0].getnewaddress(), 0.295)
        tx5 = self.nodes[3].getrawtransaction(txid5, True)
        # tx5 should have 3 inputs without grouping and 5 inputs with grouping.
        assert_equal(5 if self.options.descriptors else 3, len(tx5["vin"]))
        ...
        [self.nodes[0].sendtoaddress(addr_aps3, 0.1) for _ in range(5)]
        self.generate(self.nodes[0], 1)
        with self.nodes[4].assert_debug_log([f'Fee non-grouped = {tx5_6_ungrouped_fee}, grouped = {tx5_6_grouped_fee}, using grouped']):
            txid6 = self.nodes[4].sendtoaddress(self.nodes[0].getnewaddress(), 0.295)
        ...
        for _ in range(5):
            raw_tx = self.nodes[0].createrawtransaction([{"txid":"0"*64, "vout":0}], [{addr2[0]: 0.0005}])
        ...
        assert self.nodes[2].sendtoaddress(address=addr2[0], amount=0.05)
